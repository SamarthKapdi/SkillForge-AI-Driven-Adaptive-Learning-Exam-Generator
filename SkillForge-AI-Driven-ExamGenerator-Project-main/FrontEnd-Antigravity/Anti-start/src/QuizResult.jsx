...
const QuizResult = () => {
    ...
    const totalMarks = data?.totalMarks || data?.maxMarks || data?.totalQuestions || 100;

    // Handle case where totalMarks is 0 but we have a score (like long answers)
    const effectiveTotalMarks = totalMarks > 0 ? totalMarks : (score > 0 ? score : 100);
    const percentage = data?.percentage || Math.round((score / effectiveTotalMarks) * 100) || 0;

    // For long answers (totalMarks === 0), if score > 0, consider it passed regardless of API status
    const isPass = (totalMarks === 0 && score > 0) ||
        (data?.passStatus === "Pass" || data?.passStatus === "PASS") ||
        (percentage >= 50 && totalMarks > 0);
                                    ...
                                    onClick={() => {
                                        if (!aiSuggestions && !loadingSuggestions) {
                                            ...
                                            aiService.generateStudySuggestions({
                                                ...
                                                totalMarks,
                                                percentage,
                                                incorrectTopics: fullSummary?.questions
                                                    ?.filter(q => q.marksAwarded === 0)
