...
const QuizAttempt = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log(`[SUBMIT_DEBUG] Attempting Submission for Quiz ID: ${finalQuizId}, Student ID: ${studentId}`);

            // 1. Submit the quiz
            const response = await quizService.submitQuizAttempt(finalQuizId, answers, studentId);
            console.log("[SUBMIT_DEBUG] Submission response received.");

            // 2. Immediately fetch the results/score for the simple view
            try {
                const latestData = await quizService.getStudentLatestAttempt(finalQuizId);
                console.log("[SUBMIT_DEBUG] Fetched latest attempt data for simple view:", latestData);

                setResult({
                    score: latestData.score || latestData.totalScore || 0,
                    totalMarks: latestData.totalMarks || 100,
                    percentage: latestData.percentage || Math.round(((latestData.score || 0) / (latestData.totalMarks || 100)) * 100),
                    passStatus: latestData.passStatus,
                    fromServer: true
                });
            } catch (fetchError) {
                console.warn("[SUBMIT_DEBUG] Fallback: Could not fetch latest attempt data, using response if available.");
                // Fallback to response data if fetching failed
                setResult({
